...
def inference_on_dir(model,
    ...
    for test_id in range(0, len(filenames) - 1, stride):
        ...
        print(test_id, filenames[test_id], filenames[test_id+1])
        ...
        output_file = os.path.join(output_path, os.path.basename(filenames[test_id])[:-4] + '_flow_fwd.png')
        ...
        if save_flo_flow:
            output_file = os.path.join(output_path, os.path.basename(filenames[test_id])[:-4] + '_fwd.flo')
            frame_utils.writeFlow(output_file, flow)
            if pred_bidir_flow:
