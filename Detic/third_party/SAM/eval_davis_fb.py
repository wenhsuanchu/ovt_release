...
from sam_propagator_fb import Propagator
...
for data in progressbar(test_loader, max_value=len(test_loader), redirect_stdout=True):
    ...
    for ti in range(processor.t):
        ...
        back_prob = processor.back_prob[ti]
        if len(back_prob) > 0:
            num_instances = back_prob.shape[0]
            back_prob = torch.cat([bg_mask, back_prob], dim=0)
            back_out_labels = torch.argmax(back_prob, dim=0)
            back_replaced_labels = back_out_labels.clone()

            for idx in range(num_instances):
                #print(ti, "POST", back_replaced_labels.shape, len(processor.back_valid_instances[ti]))
                back_replaced_labels[back_out_labels==idx+1] = processor.back_valid_instances[ti][idx]
            
            replaced_labels = (~back_replaced_labels.bool()) * replaced_labels + back_replaced_labels.bool() * back_replaced_labels
